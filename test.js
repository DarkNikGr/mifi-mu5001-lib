...
async function test () {
    const session = new Session("192.168.0.1");
    await session.login('mitakidis91gr!');
    const forms = new Forms(session);

    const changeMode = await forms.change_mode('LTE_BRIDGE');
    console.log('set mode', changeMode);
